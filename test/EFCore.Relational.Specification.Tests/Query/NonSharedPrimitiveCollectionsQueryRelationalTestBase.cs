...
public abstract class NonSharedPrimitiveCollectionsQueryRelationalTestBase : NonSharedPrimitiveCollectionsQueryTestBase
{
    ...
    [ConditionalFact]
    public virtual async Task Inline_collection_Count_with_column_predicate_with_default_parameters()
    {
        var contextFactory = await InitializeAsync<TestContext>(
        ...
        // temporary EF.Parameter
        var result = await context.Set<TestEntity>().Where(c => EF.Parameter(new[] { 2, 999 }).Count(i => i > c.Id) == 1).Select(x => x.Id).ToListAsync();
        Assert.Equivalent(new[] { 100 }, result);
    }

    [ConditionalFact]
    public virtual async Task Inline_collection_of_ints_Contains_int_with_default_parameters()
    {
        var contextFactory = await InitializeAsync<TestContext>(
            onConfiguring: b => SetPrimitiveCollectionsParameters(b),
            seed: context =>
        ...
        // temporary EF.Parameter
        var result = await context.Set<TestEntity>().Where(c => EF.Parameter(new[] { 2, 999 }).Contains(c.Id)).Select(x => x.Id).ToListAsync();
        Assert.Equivalent(new[] { 2 }, result);
    }
