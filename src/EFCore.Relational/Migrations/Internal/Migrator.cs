...
public class Migrator : IMigrator
{
    ...
    public virtual void Migrate(string? targetMigration)
    {
        ...
        var useExecutionStrategy = _connection.CurrentTransaction is not null;
        ...
        try
        {
            _executionStrategy.Execute(
                ...
                static (_, migrator) =>
                {
                    ...
                    try
                    {
                        return migrator._historyRepository.CreateIfDoesNotExist();
                    }
                    finally
                    {
            ...
            _executionStrategy.Execute(
                ...
                State: new MigrationExecutionState(),
        ...
        DbContext context, string? targetMigration, MigrationExecutionState state, bool useTransaction)
    {
        ...
        try
        {
            if (useTransaction)
            {
                state.Transaction = _connection.BeginTransaction();
            }
            ...
            foreach (var commandList in commandLists)
            {
                ...
                _migrationCommandExecutor.ExecuteNonQuery(
                    getCommands(), _connection, state, useExecutionStrategy: false);
            }
        ...
        try
        {
            await _executionStrategy.ExecuteAsync(
                ...
                static async (_, migrator, ct) =>
                {
                    ...
                    try
                    {
                        return await migrator._historyRepository.CreateIfDoesNotExistAsync(ct).ConfigureAwait(false);
                    }
                    finally
                    {
        ...
        DbContext context, string? targetMigration, MigrationExecutionState state, bool useTransaction, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (useTransaction)
            {
                state.Transaction = await context.Database.BeginTransactionAsync(cancellationToken).ConfigureAwait(false);
            }
            ...
            foreach (var commandList in commandLists)
            {
                ...
                await _migrationCommandExecutor.ExecuteNonQueryAsync(
                    getCommands(), _connection, state, useExecutionStrategy: false, cancellationToken)
                    .ConfigureAwait(false);
            }
