...
public interface IMigrationsDatabaseLock : IDisposable, IAsyncDisposable
{
    ...
    /// </summary>
    LockReleaseBehavior ReleaseBehavior { get; }
    ...
    {
        if ((connectionReopened && ReleaseBehavior == LockReleaseBehavior.Connection)
                || (transactionRestarted is true && ReleaseBehavior == LockReleaseBehavior.Transaction))
        {
            Dispose();
